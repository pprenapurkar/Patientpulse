...
async def get_patient_context(patient_id: str, request: Request):
    ...
    # Stamp freshness so frontend can show FHIR fallback banner
    context.data_freshness = source  # type: ignore[assignment]
    # by_alias=True + mode="json" → camelCase keys + datetime as ISO strings
